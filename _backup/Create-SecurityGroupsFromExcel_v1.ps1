...
# PowerShell script to create and manage AWS security groups and rules from Excel configuration using AWSPowerShell.NetCore with multiple SSO profiles
...
# Uses -IpPermission (singular) for Grant-EC2SecurityGroupIngress/Egress to match older module versions
...
try {
    ...
    try {
        ...
        $moduleVersion = (Get-Module -Name AWSPowerShell.NetCore).Version.ToString()
        $availableParams = (Get-Command Grant-EC2SecurityGroupIngress).Parameters.Keys -join ", "
        Write-Log "Loaded AWSPowerShell.NetCore version: $moduleVersion" "INFO"
        ...
        if ($DryRun) {
            ...
            try {
                ...
                if ($existingGroups.Count -gt 0) {
                    ...
                    $securityGroupId = New-EC2SecurityGroup -GroupName $groupName -Description $groupDescription -VpcId $vpcId -ProfileName $profileName -Region $region -ErrorAction Stop
            ...
            try {
                        ...
                        foreach ($rule in $rules) {
                            if ($rule.IpProtocol -eq $config.Protocol -and
                                ...
                                if ($config.Source -match '^sg-' -and $rule.UserIdGroupPairs.GroupId -contains $config.Source) {
                                    $ruleExists = $true
                                    # UserIdGroupPairs don't support descriptions in older module versions
                                    break
                                } elseif ($rule.IpRanges -contains $config.Source) {
                                    # Special handling for default egress rule (all -1 -1 0.0.0.0/0)
                                    if ($config.RuleType -eq 'egress' -and 
                                        ...
                                        $ruleExists = $true
                                        Write-Log "Default egress rule (all -1 -1 0.0.0.0/0) exists for group '$groupName'. Skipping addition." "INFO"
                                        break
                                    }
                                    $ruleExists = $true
                                    # Description updates not supported in older module versions
                                    break
                                }
                            }
                        }
                ...
                if ($DryRun) {
                    ...
                    try {
                        ...
                        if ($config.Source -match '^sg-') {
                            $params.Add('SourceSecurityGroupName', $config.Source)
                        } else {
                            $params.Add('Cidr', $config.Source)
                        }
