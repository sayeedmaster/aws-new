...
# Prerequisites:
# 1. AWSPowerShell.NetCore module located at the specified PSModulesPath
...
# 4. Excel file with EC2 configuration. By default, the script looks for 'EC2_Config.xlsx' in its own directory.
# 5. SSO profiles in ~/.aws/config with format: [profile sso-<AccountName>-<SSORole>]
...
try {
    ...
    # Import required modules from the specified path
    try {
    ...
    foreach ($config in $ec2Configs) {
        try {
            ...
            $validRegions = Get-AWSRegion -ErrorAction Stop | Select-Object -ExpandProperty Region
