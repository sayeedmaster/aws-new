...
function Test-SecurityGroupPermissions {
    ...
    try {
        ...
        # Test ec2:DescribeSecurityGroups permission
        ...
        if ($TestDeletePermission) {
            # Test ec2:DeleteSecurityGroup permission with a randomized valid group ID
...
function Confirm-SecurityGroupRules {
    ...
    try {
        ...
        Write-Log "Verifying rules for security group '$GroupName' (ID: $GroupId):"
...
try {
    ...
    Write-Log "Starting security group creation script (DryRun: $DryRun, ReplaceSG: $ReplaceSG, SkipPermissionValidation: $SkipPermissionValidation)"
    ...
    foreach ($group in $groupedConfigs) {
        ...
        # Validate permissions for security group operations (unless skipped)
        ...
        if ($DryRun) {
            ...
            try {
                ...
                if ($existingGroups.Count -eq 1) {
                    ...
                    if ($ReplaceSG) {
                        Write-Log "ReplaceSG specified. Attempting to delete security group '$groupName' (ID: $securityGroupId)."
                        try {
                            Remove-EC2SecurityGroup -GroupId $securityGroupId -ProfileName $profileName -Region $region -Force -ErrorAction Stop
                            Write-Log "Successfully deleted security group '$groupName' (ID: $securityGroupId)."
                            $securityGroupId = $null
                            $isNewGroup = $true
                        } catch {
                ...
                # Create new security group if it doesn't exist or was deleted
                if (-not $securityGroupId) {
                    Write-Log "Security group '$groupName' does not exist in VPC $vpcId or was deleted. Creating new security group."
                    $maxRetries = 3
                    $retryCount = 0
                    $newGroup = $null
        ...
        foreach ($config in $group.Group) {
            try {
                ...
                # Skip this check if -ReplaceSG is specified since the group is new
                $ruleExists = $false
        ...
        Write-Log "Processed $rulesProcessed rules for group '$groupName' (ID: $securityGroupId), successfully added $rulesAdded rules."
