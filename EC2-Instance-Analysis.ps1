...
<#
.SYNOPSIS
    Enhanced EC2 Instance Analysis with AWS Systems Manager (SSM) integration.
    
.DESCRIPTION
    This script performs comprehensive EC2 instance analysis across multiple AWS accounts and regions,
    now enhanced with AWS Systems Manager data including:
    - SSM managed instance status and health
    - SSM agent version and last ping time
    - Detailed OS information from SSM
    - Patch compliance status
    - Platform details (Windows/Linux versions)
...
.PARAMETER DebugPlatform
    Enables additional platform debugging information in output.
...
.PARAMETER FilterProblematicTags
    Filters out known problematic EC2 tags from the output (default: $true).
    
.PARAMETER TestProfilesFirst
    Tests profile connectivity before processing (default: $true).
    
.PARAMETER IncludeSSMData
    Enables collection of AWS Systems Manager data for managed instances (default: $true).
    
.PARAMETER IncludePatchCompliance
    Enables collection of patch compliance data for SSM managed instances (default: $false).
    
.EXAMPLE
    .\EC2-Instance-Analysis.ps1 -PSModulesPath "C:\ProgramFiles\AWS Tools\PowerShell"
    Runs analysis with SSM data collection but without patch compliance (default behavior).
    
.EXAMPLE
    .\EC2-Instance-Analysis.ps1 -PSModulesPath "C:\ProgramFiles\AWS Tools\PowerShell" -IncludePatchCompliance $true
    Runs analysis with SSM data collection AND patch compliance data.
    
.EXAMPLE
    .\EC2-Instance-Analysis.ps1 -PSModulesPath "C:\ProgramFiles\AWS Tools\PowerShell" -IncludeSSMData $false
    Runs analysis without SSM data collection (fastest execution).
    
.EXAMPLE
    .\EC2-Instance-Analysis.ps1 -PSModulesPath "C:\ProgramFiles\AWS Tools\PowerShell" -Region us-east-1 -AwsProfiles "profile1","profile2"
    Runs analysis for specific profiles and region with SSM data but no patch compliance.
    
.NOTES
    Version: 14.0 (Enhanced with SSM data integration)
    Author: Enhanced for SSM integration
    Requires: AWS.Tools.EC2, AWS.Tools.SimpleSystemsManagement, AWS.Tools.Common, AWS.Tools.SecurityToken, AWS.Tools.IdentityManagement
#>
...
param(
    ...
    [Parameter()]
    [bool]$IncludeSSMData = $true,
    [Parameter()]
    [bool]$IncludePatchCompliance = $false
)
...
$ssmCache = [System.Collections.Concurrent.ConcurrentDictionary[string, object]]::new()
$patchComplianceCache = [System.Collections.Concurrent.ConcurrentDictionary[string, object]]::new()
$maxParallelJobs = 5  # Configurable throttle limit
$scriptVersion = "14.0 (Enhanced with SSM data integration)"
...
try {
    $modulePaths = @(
        ...
        (Join-Path $PSModulesPath "AWS.Tools.SimpleSystemsManagement")
    )
...
$results = $AwsProfiles | ForEach-Object -Parallel {
    ...
    $local_ssmCache = $using:ssmCache
    $local_patchComplianceCache = $using:patchComplianceCache
    $local_LogFilePath = $using:LogFilePath
    $local_ProblematicTags = $using:ProblematicTags
    $local_DebugPlatform = $using:DebugPlatform
    $local_IncludeSSMData = $using:IncludeSSMData
    $local_IncludePatchCompliance = $using:IncludePatchCompliance
    ...
    # Define Get-SSMInstanceData function
    function Get-SSMInstanceData {
        param([string]$InstanceId, [string]$ProfileName, [string]$Region, [hashtable]$SSMCache)
        $cacheKey = "${ProfileName}:${Region}:${InstanceId}"
        [System.Threading.Monitor]::Enter($SSMCache)
        try {
            if ($SSMCache.ContainsKey($cacheKey)) { 
                return $SSMCache[$cacheKey] 
            }
        } finally { 
            [System.Threading.Monitor]::Exit($SSMCache) 
        }
        
        try {
            # Get SSM instance information
            $ssmInstance = Invoke-AwsApiCall -ApiCall { 
                Get-SSMInstanceInformation -InstanceInformationFilterList @{Key="InstanceIds";ValueSet=@($InstanceId)} -ProfileName $ProfileName -Region $Region -ErrorAction Stop 
            } -ProfileName $ProfileName -Region $Region -OperationDescription "Get SSM instance info for $InstanceId"
            
            if ($ssmInstance -and $ssmInstance.Count -gt 0) {
                $ssmInfo = $ssmInstance[0]
                $ssmData = @{
                    SSMManaged = $true
                    SSMAgentVersion = $ssmInfo.AgentVersion ?? "N/A"
                    SSMLastPingTime = $ssmInfo.LastPingDateTime ?? "N/A"
                    SSMPingStatus = $ssmInfo.PingStatus ?? "N/A"
                    SSMPlatformType = $ssmInfo.PlatformType ?? "N/A"
                    SSMPlatformName = $ssmInfo.PlatformName ?? "N/A"
                    SSMPlatformVersion = $ssmInfo.PlatformVersion ?? "N/A"
                    SSMInstanceId = $ssmInfo.InstanceId ?? $InstanceId
                    OSDetails = "$($ssmInfo.PlatformName ?? 'Unknown') $($ssmInfo.PlatformVersion ?? '')".Trim()
                }
            } else {
                $ssmData = @{
                    SSMManaged = $false
                    SSMAgentVersion = "N/A"
                    SSMLastPingTime = "N/A"
                    SSMPingStatus = "Not Managed"
                    SSMPlatformType = "N/A"
                    SSMPlatformName = "N/A"
                    SSMPlatformVersion = "N/A"
                    SSMInstanceId = $InstanceId
                    OSDetails = "N/A"
                }
            }
        } catch {
            Write-Log "Failed to get SSM data for instance ${InstanceId}: $($_.Exception.Message)" "WARN"
            $ssmData = @{
                SSMManaged = $false
                SSMAgentVersion = "Error"
                SSMLastPingTime = "Error"
                SSMPingStatus = "Error"
                SSMPlatformType = "Error"
                SSMPlatformName = "Error"
                SSMPlatformVersion = "Error"
                SSMInstanceId = $InstanceId
                OSDetails = "Error"
            }
        }
    ...
    # Define Get-SSMPatchCompliance function
    function Get-SSMPatchCompliance {
        param([string]$InstanceId, [string]$ProfileName, [string]$Region, [hashtable]$PatchComplianceCache)
        $cacheKey = "${ProfileName}:${Region}:${InstanceId}"
        [System.Threading.Monitor]::Enter($PatchComplianceCache)
        try {
            if ($PatchComplianceCache.ContainsKey($cacheKey)) { 
                return $PatchComplianceCache[$cacheKey] 
            }
        } finally { 
            [System.Threading.Monitor]::Exit($PatchComplianceCache) 
        }
        
        try {
            # Use Get-SSMInstancePatchState - specifically designed for patch compliance
            try {
                $patchState = Invoke-AwsApiCall -ApiCall { 
                    Get-SSMInstancePatchState -InstanceId $InstanceId -ProfileName $ProfileName -Region $Region -ErrorAction Stop 
                } -ProfileName $ProfileName -Region $Region -OperationDescription "Get patch compliance for $InstanceId"
                
                if ($patchState -and $patchState.Count -gt 0) {
                    # Get the most recent patch state
                    $latestPatchState = $patchState | Sort-Object OperationEndTime -Descending | Select-Object -First 1
                    $complianceData = @{
                        PatchCompliance = $latestPatchState.Operation ?? "Unknown"
                        PatchComplianceDetails = @{
                            InstalledCount = $latestPatchState.InstalledCount ?? 0
                            MissingCount = $latestPatchState.MissingCount ?? 0
                            FailedCount = $latestPatchState.FailedCount ?? 0
                            NotApplicableCount = $latestPatchState.NotApplicableCount ?? 0
                        }
                        LastComplianceCheck = $latestPatchState.OperationEndTime ?? "N/A"
                    }
                } else {
                    throw "No patch state data returned"
                }
            } catch {
                # Fallback 1: Try Get-SSMComplianceItemList
                try {
                    $complianceItems = Invoke-AwsApiCall -ApiCall { 
                        Get-SSMComplianceItemList -ResourceId $InstanceId -ResourceType "ManagedInstance" -ComplianceType "Patch" -ProfileName $ProfileName -Region $Region -ErrorAction Stop 
                    } -ProfileName $ProfileName -Region $Region -OperationDescription "Get patch compliance items for $InstanceId"
                    
                    if ($complianceItems -and $complianceItems.Count -gt 0) {
                        $latestCompliance = $complianceItems | Sort-Object ExecutionSummary.ExecutionTime -Descending | Select-Object -First 1
                        $complianceData = @{
                            PatchCompliance = $latestCompliance.Status ?? "Unknown"
                            PatchComplianceDetails = $latestCompliance.Details ?? @{}
                            LastComplianceCheck = $latestCompliance.ExecutionSummary.ExecutionTime ?? "N/A"
                        }
                    } else {
                        throw "No compliance items found"
                    }
                } catch {
                    # Final fallback: Return Not Available
                    Write-Log "Patch compliance not available for instance: $InstanceId" "WARN"
                    $complianceData = @{
                        PatchCompliance = "Not Available"
                        PatchComplianceDetails = @{}
                        LastComplianceCheck = "Not Available"
                    }
                }
            }
        } catch {
            Write-Log "Failed to get patch compliance for instance ${InstanceId}: $($_.Exception.Message)" "WARN"
            $complianceData = @{
                PatchCompliance = "Error"
                PatchComplianceDetails = @{}
                LastComplianceCheck = "Error"
            }
        }
        
        [System.Threading.Monitor]::Enter($PatchComplianceCache)
        try { 
            $PatchComplianceCache[$cacheKey] = $complianceData 
        } finally { 
            [System.Threading.Monitor]::Exit($PatchComplianceCache) 
        }
    ...
    function Get-EC2InstancesForProfile {
        param([string]$ProfileName, [string]$Region, [bool]$FilterTags, [hashtable]$SubnetCache, [hashtable]$VpcCache, [hashtable]$AccountProfileCache, [string[]]$AvailableProfiles, [hashtable]$InstanceIdsProcessed, [hashtable]$AttributeCache, [System.Collections.Concurrent.ConcurrentBag[PSObject]]$OutputBag, [hashtable]$SSMCache, [hashtable]$PatchComplianceCache)
        ...
        foreach ($instance in $instances) {
            ...
            try {
                if ($InstanceIdsProcessed.ContainsKey($instance.InstanceId)) { continue }
            ...
            # Get SSM data if enabled
            $ssmData = if ($local_IncludeSSMData) { 
                Get-SSMInstanceData -InstanceId $instance.InstanceId -ProfileName $ProfileName -Region $Region -SSMCache $SSMCache 
            } else { 
                @{
                    SSMManaged = "Disabled"
                    SSMAgentVersion = "Disabled"
                    SSMLastPingTime = "Disabled"
                    SSMPingStatus = "Disabled"
                    SSMPlatformType = "Disabled"
                    SSMPlatformName = "Disabled"
                    SSMPlatformVersion = "Disabled"
                    SSMInstanceId = "Disabled"
                    OSDetails = "Disabled"
                }
            }
            
            # Get patch compliance data if enabled
            $patchData = if ($local_IncludePatchCompliance -and $ssmData.SSMManaged -eq $true) {
                Get-SSMPatchCompliance -InstanceId $instance.InstanceId -ProfileName $ProfileName -Region $Region -PatchComplianceCache $PatchComplianceCache
            } else {
                @{
                    PatchCompliance = if ($local_IncludePatchCompliance) { "Not SSM Managed" } else { "Disabled" }
                    PatchComplianceDetails = @{}
                    LastComplianceCheck = if ($local_IncludePatchCompliance) { "Not SSM Managed" } else { "Disabled" }
                }
            }
            ...
            $tempProps = [ordered]@{
                ...
                # SSM Related Fields
                SSMManaged = $ssmData.SSMManaged
                SSMAgentVersion = $ssmData.SSMAgentVersion
                SSMLastPingTime = $ssmData.SSMLastPingTime
                SSMPingStatus = $ssmData.SSMPingStatus
                OSDetails = $ssmData.OSDetails
                SSMPlatformType = $ssmData.SSMPlatformType
                SSMPlatformName = $ssmData.SSMPlatformName
                SSMPlatformVersion = $ssmData.SSMPlatformVersion
                # Patch Compliance Fields
                PatchCompliance = $patchData.PatchCompliance
                LastComplianceCheck = $patchData.LastComplianceCheck
        ...
        try {
            ...
            if (Test-Path $configPath) {
                ...
                if ($configContent -match "(?s)$profileSection.*?\nsso_role_name\s*=\s*([^\s#\r\n]+)") { 
                    ...
                    # Validate that we got a proper role name (not just "profile" or other invalid values)
                    if ($ssoRoleName -and $ssoRoleName -ne "profile" -and $ssoRoleName -ne $ProfileName -and $ssoRoleName.Length -gt 1) {
                        return $ssoRoleName
                    } else {
                        Write-Log "Invalid SSO role name '$ssoRoleName' found in config for profile $ProfileName, trying alternative methods" "WARN"
                    }
                }
            } else {
            ...
            if ($identity -and $identity.Arn) {
                ...
                Write-Log "Failed to get valid identity or ARN for profile $ProfileName" "WARN"
            }
            return $ProfileName
        } catch { 
    ...
    try {
        ...
        if ($local_IncludeSSMData -or $local_IncludePatchCompliance) {
            Import-Module -Name (Join-Path $local_PSModulesPath "AWS.Tools.SimpleSystemsManagement") -ErrorAction Stop
        }
    } catch {
    ...
    try {
        $profileInstances, $accountName, $accountId, $amiReport, $accountInfo, $regions = Get-EC2InstancesForProfile -ProfileName $profileName -Region $currentRegion -FilterTags $local_FilterProblematicTags -SubnetCache $local_subnetCache -VpcCache $local_vpcCache -AccountProfileCache $local_accountProfileCache -AvailableProfiles $local_AwsProfiles -InstanceIdsProcessed $local_instanceIdsProcessed -AttributeCache $local_attributeCache -OutputBag $local_outputBag -SSMCache $local_ssmCache -PatchComplianceCache $local_patchComplianceCache
...
if ($outputBag.Count -gt 0) {
    $props = [ordered]@{ 
        AccountName = $null; AccountId = $null; SSORole = $null; InstanceId = $null; InstanceName = $null; Tenancy = $null; 
        Monitored = $null; IpfEnvironment = $null; SqlServerMonitored = $null; IpfServiceName = $null; FcmsCustomMonitoring = $null; 
        Role = $null; Application = $null; PatchGroup = $null; AutoPatch = $null; InstanceState = $null; AvailabilityZone = $null; 
        VpcId = $null; VpcOwnerId = $null; VpcIsShared = $null; SubnetId = $null; SubnetName = $null; CidrBlock = $null; 
        PrivateIpAddress = $null; SecondaryPrivateIPs = $null; Platform = $null; InstanceType = $null; CWMonitoring = $null; 
        vCPU = $null; AMIId = $null; AMIName = $null; IamInstanceProfile = $null; EbsOptimized = $null; 
        MetadataOptionsHttpTokens = $null; MetadataOptionsHttpEndpoint = $null; MetadataOptionsHttpPutResponseHopLimit = $null; 
        InstanceMetadataTags = $null; DisableApiTermination = $null; InstanceInitiatedShutdownBehavior = $null; KeyPair = $null; 
        SecurityGroupIds = $null; 
        # SSM Related Fields
        SSMManaged = $null; SSMAgentVersion = $null; SSMLastPingTime = $null; SSMPingStatus = $null; OSDetails = $null; 
        SSMPlatformType = $null; SSMPlatformName = $null; SSMPlatformVersion = $null; 
        # Patch Compliance Fields
        PatchCompliance = $null; LastComplianceCheck = $null; 
        Tags = $null; TagsFiltered = $null; ProblematicTags = $null 
    }
...
Write-Host "SSM data collection: $IncludeSSMData" -ForegroundColor Green
Write-Host "Patch compliance collection: $IncludePatchCompliance" -ForegroundColor Green
...
if ($IncludeSSMData -and $outputBag.Count -gt 0) {
    $ssmManagedCount = ($outputBag | Where-Object { $_.SSMManaged -eq $true }).Count
    $ssmNotManagedCount = ($outputBag | Where-Object { $_.SSMManaged -eq $false }).Count
    Write-Host "SSM Managed instances: $ssmManagedCount" -ForegroundColor Green
    Write-Host "Not SSM Managed instances: $ssmNotManagedCount" -ForegroundColor Yellow
}
if ($IncludePatchCompliance -and $outputBag.Count -gt 0) {
    $patchCompliantCount = ($outputBag | Where-Object { $_.PatchCompliance -eq "COMPLIANT" }).Count
    $patchNonCompliantCount = ($outputBag | Where-Object { $_.PatchCompliance -eq "NON_COMPLIANT" }).Count
    Write-Host "Patch compliant instances: $patchCompliantCount" -ForegroundColor Green
    Write-Host "Patch non-compliant instances: $patchNonCompliantCount" -ForegroundColor Red
}
