...
# Function to write logs with buffering
function Write-Log {
    param (
        [string]$Message,
        [string]$Level = "INFO"
    ...
    if ($script:logBuffer.Count -ge 100) {
        ...
        try {
            Add-Content -Path $LogFilePath -Value $script:logBuffer -ErrorAction SilentlyContinue
...
# Function to execute AWS API calls with retries
function Invoke-AwsApiCall {
    ...
    while ($retryCount -lt $MaxRetries) {
        try {
            $result = & $ApiCall
            Write-Log "Successfully executed ${OperationDescription}" "INFO"
            return $result
        } catch {
            $retryCount++
            Write-Log "Retry $retryCount/$MaxRetries for ${OperationDescription}: $($_.Exception.Message)" "WARN"
            if ($retryCount -eq $MaxRetries) {
                Write-Log "Failed ${OperationDescription} after $MaxRetries retries: $($_.Exception.Message)" "ERROR"
                return $null
            }
...
function Sanitize-String {
    ...
    try {
        ...
        Write-Log "Sanitized string '${InputString}' to '${sanitized}'" "INFO"
        return $sanitized
    } catch {
        Write-Log "Error sanitizing string '${InputString}': $($_.Exception.Message)" "ERROR"
        return $InputString
    }
}

# Function to validate EC2 tag keys
...
function Get-ResourceName {
    ...
    try {
        ...
        Write-Log "Error accessing tags for resource: $($_.Exception.Message)" "WARN"
        return "(No Name Tag)"
    }
}
...
function Get-ResourceTagValue {
    ...
    try {
        ...
        Write-Log "Error accessing ${TagName} tag: $($_.Exception.Message)" "WARN"
        return "N/A"
    }
}
...
function Get-InstancePlatform {
    ...
    try {
        if ($Instance.Platform -and $Instance.Platform -ne "") {
            return $Instance.Platform.ToString().ToLower()
        }
...
# Function to get AMI details with caching
...
function Get-AMIName {
    ...
    if ($amiCache.ContainsKey($cacheKey)) {
        return $amiCache[$cacheKey].Name
    }
    try {
        ...
        $amiInfo = @{
            Name = $ami[0].Name ?? "AMI No Name (${AmiId})"
            Description = $ami[0].Description ?? 'N/A'
            Architecture = $ami[0].Architecture ?? 'N/A'
            Platform = $ami[0].PlatformDetails ?? 'N/A'
            CreationDate = $ami[0].CreationDate ?? 'N/A'
            OwnerId = $ami[0].OwnerId ?? 'N/A'
            State = $ami[0].State ?? 'N/A'
            Public = $ami[0].Public ?? $false
        }
        $amiCache.TryAdd($cacheKey, $amiInfo)
        return $amiInfo.Name
    } catch {
...
# Function to preload subnet and VPC data
function Initialize-AwsResourceCache {
    ...
    Write-Log "Preloading subnet and VPC data for profile ${ProfileName} in region ${Region}" "INFO"
    try {
        ...
        foreach ($subnet in $subnets) {
            ...
            try {
                $SubnetCache[$cacheKey] = @{
                    CidrBlock = $subnet.CidrBlock ?? 'N/A'
                    SubnetName = ($subnet.Tags | Where-Object { $_.Key -eq 'Name' } | Select-Object -ExpandProperty Value) ?? 'N/A'
                }
            } finally {
                [System.Threading.Monitor]::Exit($SubnetCache)
            }
        }
        ...
        foreach ($vpc in $vpcs) {
            ...
            try {
                $VpcCache[$cacheKey] = $vpc.OwnerId ?? 'N/A'
            } finally {
                [System.Threading.Monitor]::Exit($VpcCache)
            }
        }
        Write-Log "Cached $($subnets.Count) subnets and $($vpcs.Count) VPCs for profile ${ProfileName}" "INFO"
    } catch {
        Write-Log "Failed to preload resources for profile ${ProfileName}: $($_.Exception.Message)" "ERROR"
    }
}
...
function Get-SubnetCidrBlock {
    ...
    if ($IsSharedVPC -and $VpcOwnerId -and $VpcOwnerId -ne 'N/A') {
        ...
        if ($ownerProfile) {
            $profilesToTry = @($ownerProfile) + $profilesToTry
        }
    }
...
function Get-VpcOwnerId {
    ...
    [System.Threading.Monitor]::Enter($VpcCache)
    try {
        $VpcCache[$cacheKey] = "VPC Query Error (${VpcId})"
    } finally {
        [System.Threading.Monitor]::Exit($VpcCache)
    }
    return $VpcCache[$cacheKey]
...
# Function to get instance attributes
function Get-InstanceAttributes {
    ...
    try {
        ...
        [System.Threading.Monitor]::Exit($AttributeCache)
    }
    try {
        ...
        $attributes = @{
            DisableApiTermination = $disableApiTermination ?? "N/A"
            InstanceInitiatedShutdownBehavior = $shutdownBehavior ?? "N/A"
        }
        [System.Threading.Monitor]::Enter($AttributeCache)
        try {
        ...
        return @{ DisableApiTermination = "N/A"; InstanceInitiatedShutdownBehavior = "N/A" }
    }
}
...
function Get-AmiUsageReport {
    ...
    foreach ($amiId in $uniqueAmis) {
        ...
        $cacheKey = "${ProfileName}:${Region}:${amiId}"
        $amiInfo = $amiCache[$cacheKey] ?? @{ Name = "AMI Query Error (${amiId})"; Description = 'Error'; Architecture = 'N/A'; Platform = 'N/A'; CreationDate = 'N/A'; OwnerId = 'N/A'; State = 'N/A'; Public = $false }
        ...
        $amiReport += [PSCustomObject]@{
            ...
            Architecture = $amiInfo.Architecture
            Platform = $amiInfo.Platform
            CreationDate = $amiInfo.CreationDate
            OwnerId = $amiInfo.OwnerId
            State = $amiInfo.State
    ...
    Write-Log "Generated AMI report with $($amiReport.Count) unique AMIs for profile ${ProfileName}" "INFO"
    return $amiReport
}
...
function Get-EC2InstancesForProfile {
    param(
        ...
        [hashtable]$AccountProfileCache,
        [string[]]$AvailableProfiles,
        [hashtable]$InstanceIdsProcessed,
        [hashtable]$AttributeCache,
        [string]$OutputFile,
        [System.Collections.Concurrent.ConcurrentBag[PSObject]]$OutputBag
    )
    ...
    try {
        ...
        if (-not $accountName) {
            $accountName = if ($ProfileName) { $ProfileName -replace '^sso-' -replace '-nonprivFujitsuCSA$', '' } else { 'Default' }
            Write-Log "No account alias found. Using '${accountName}' as AccountName." "INFO"
        }
        Write-Log "Account: ${accountName} (${accountId})" "INFO"
    } catch {
        Write-Log "Failed to configure credentials for profile ${DisplayProfileName}: $($_.Exception.Message)" "ERROR"
        return @(), $accountName, $accountId, @(), @(), @()
    }
    # Preload subnet and VPC data
    ...
    $profileInstances = @()
    $progress = 0
    # Initialize CSV with headers if not already done
    if (-not (Test-Path $OutputFile)) {
        $props = [ordered]@{
            ...
            SecurityGroupIds = $null; Tags = $null; TagsFiltered = $null; ProblematicTags = $null
        }
        if ($DebugPlatform) {
            $props.Add("RawPlatform", $null)
            $props.Add("RawPlatformDetails", $null)
        }
        [System.Threading.Monitor]::Enter($OutputFile)
        try {
            $props | Select-Object * | Export-Csv -Path $OutputFile -NoTypeInformation -Encoding UTF8 -Force
        } finally {
    ...
    foreach ($instance in $instances) {
        ...
        try {
            if ($InstanceIdsProcessed.ContainsKey($instance.InstanceId)) {
                Write-Log "Instance $($instance.InstanceId) already processed. Skipping." "DEBUG"
                continue
            }
            $InstanceIdsProcessed[$instance.InstanceId] = $true
        } finally {
            [System.Threading.Monitor]::Exit($InstanceIdsProcessed)
        }
        try {
            $tags = if ($FilterTags) { $instance.Tags | Where-Object { $_.Key -notin $ProblematicTags } } else { $instance.Tags }
            ...
            $props = [ordered]@{
                ...
                VpcIsShared = if ($instance.VpcId -and $VpcOwnerId -ne 'N/A' -and $VpcOwnerId -ne "VPC Query Error ($($instance.VpcId))" -and $VpcOwnerId -ne $accountId) { $true } else { $false }
            ...
            $instanceObj = [PSCustomObject]$props
            $profileInstances += $instanceObj
            $OutputBag.Add($instanceObj)
            [System.Threading.Monitor]::Enter($OutputFile)
            try {
                $instanceObj | Export-Csv -Path $OutputFile -NoTypeInformation -Encoding UTF8 -Append
            } finally {
                [System.Threading.Monitor]::Exit($OutputFile)
            }
        } catch {
...
function Select-AwsProfilesInteractive {
    ...
    if ($ProfileList.Count -eq 0) {
        Write-Log "No profiles available for selection" "WARN"
        exit 0
    }
    try {
        $selected = $ProfileList | Out-GridView -Title "Select AWS Profiles" -OutputMode Multiple
        if (-not $selected) {
            Write-Log "No profiles selected. Exiting." "WARN"
            exit 0
        }
        Write-Log "Selected profiles: $($selected -join ', ')" "INFO"
        return $selected
    } catch {
        Write-Log "Out-GridView not available. Falling back to manual selection." "WARN"
        $selectedProfiles = @()
        $currentSelection = 0
        $marked = @{}
        ...
        do {
            ...
            switch ($key.VirtualKeyCode) {
                ...
                27 { Write-Log "Profile selection cancelled." "WARN"; exit 0 }
            }
        } while ($true)
    }
}
...
function Test-AwsProfileConnectivity {
    ...
    try {
        Invoke-AwsApiCall -ApiCall { Get-STSCallerIdentity -ProfileName $ProfileName -Region $Region -ErrorAction Stop } -ProfileName $ProfileName -Region $Region -OperationDescription "Test profile $displayProfileName" | Out-Null
...
try {
    ...
    if (-not $OutputFile) {
        ...
        $regionSafe = if ($Region) { Sanitize-String -InputString $Region } else { "multiregion" }
        $filterSuffix = if ($FilterProblematicTags) { "_filtered" } else { "_unfiltered" }
        $OutputFile = Join-Path $OutputDir "ec2_instances_$($AwsProfiles.Count)accounts_${regionSafe}${filterSuffix}_${timestamp}.csv"
    ...
    Write-Log "Tag filtering: $(if ($FilterProblematicTags) { "ENABLED ($($ProblematicTags -join ', '))" } else { "DISABLED" })" "INFO"
    ...
    if ($TestProfilesFirst -and $AwsProfiles) {
        ...
        foreach ($profile in $AwsProfiles) {
            ...
            if (Test-AwsProfileConnectivity -ProfileName $profile -Region $currentRegion) {
                ...
                $identity = Invoke-AwsApiCall -ApiCall { Get-STSCallerIdentity -ProfileName $profile -Region $currentRegion -ErrorAction Stop } -ProfileName $profile -Region $currentRegion -OperationDescription "Map account for $profile"
                if ($identity) {
                    $accountProfileCache.TryAdd("$($identity.Account):$currentRegion", $profile)
                    Write-Log "Mapped account $($identity.Account) in region ${currentRegion} to profile ${profile}" "INFO"
                }
            }
        }
        if ($validProfiles.Count -eq 0) {
            Write-Log "No valid profiles found. Exiting." "ERROR"
            exit 1
        }
    ...
    # Process profiles in parallel with enhanced error handling
    $results = $AwsProfiles | ForEach-Object -Parallel {
        # Define local variables from the calling scope to avoid issues with $using in expressions
        ...
        # Import required modules in each runspace
        ...
        try {
            ...
            $timestamp = Get-Date -Format "yyyy-MM-dd HH:mm:ss"
            $logMessage = "[$timestamp] [ERROR] Failed to import AWS Tools modules in parallel runspace for profile ${profileName}: $($_.Exception.Message)"
        ...
        if (-not $currentRegion) {
            $timestamp = Get-Date -Format "yyyy-MM-dd HH:mm:ss"
            $logMessage = "[$timestamp] [ERROR] No valid region for profile $profileName. Skipping."
            [System.Threading.Monitor]::Enter($local_logBuffer)
            try {
                $local_logBuffer.Add($logMessage)
            } finally {
        ...
        $timestamp = Get-Date -Format "yyyy-MM-dd HH:mm:ss"
        $logMessage = "[$timestamp] [INFO] Processing region $currentRegion for profile $profileName"
        [System.Threading.Monitor]::Enter($local_logBuffer)
        try {
            $local_logBuffer.Add($logMessage)
        } finally {
        ...
        try {
            $profileInstances, $accountName, $accountId, $amiReport, $accountInfo, $regions = Get-EC2InstancesForProfile -ProfileName $profileName -Region $currentRegion -FilterTags $local_FilterProblematicTags -SubnetCache $local_subnetCache -VpcCache $local_vpcCache -AccountProfileCache $local_accountProfileCache -AvailableProfiles $local_AwsProfiles -InstanceIdsProcessed $local_instanceIdsProcessed -AttributeCache $local_attributeCache -OutputFile $local_OutputFile -OutputBag $local_outputBag
            ...
            $timestamp = Get-Date -Format "yyyy-MM-dd HH:mm:ss"
            $logMessage = "[$timestamp] [ERROR] Unexpected error processing profile ${profileName}: $($_.Exception.Message)"
            [System.Threading.Monitor]::Enter($local_logBuffer)
            try {
                $local_logBuffer.Add($logMessage)
            } finally {
    ...
    foreach ($result in $results) {
        ...
        # Unpack properties from the result object
        ...
        if ($profileInstances) {
            foreach ($instance in $profileInstances) {
                $outputBag.Add($instance)
            }
        }
        if ($amiReport) {
            foreach ($report in $amiReport) {
                $allAmiReports.Add($report)
            }
        }
        if ($accountInfo) {
            $processedAccounts.Add($accountInfo)
        }
        foreach ($region in $regions) {
            if (-not ($regionsUsed.Contains($region))) {
                $regionsUsed.Add($region)
            }
        }
    }
    if ($failedProfiles -gt 0) {
        Write-Log "$failedProfiles profile(s) failed to process in parallel block" "WARN"
    }
    ...
    if ($allAmiReports.Count -gt 0) {
        try {
            ...
            Write-Log "Exported AMI usage report to ${AmiOutputFile}" "INFO"
        } catch {
    ...
    Write-Host "============================" -ForegroundColor Cyan
    Write-Host "Script version: 13.7 (Fixed Using expression errors in post-parallel processing)" -ForegroundColor Green
    ...
    foreach ($account in $processedAccounts | Sort-Object AccountName) {
        Write-Log "Account: $($account.AccountName) ($($account.AccountId)): $($account.SSORole)" "INFO"
    }
    if ($allAmiReports.Count -gt 0) {
