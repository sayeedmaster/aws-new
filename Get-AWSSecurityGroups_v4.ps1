<#
.SYNOPSIS
    Retrieves all security groups in AWS accounts and checks where they are attached (EC2 instances, ENIs, ELBs, FSx, API Gateway VPC endpoints, or Transit Gateway attachments) with counts.
.DESCRIPTION
    This script queries AWS security groups in all regions using a single API call per profile/region, caches resource data to minimize API calls, and determines if each security group is attached to supported resources or is orphaned, including counts of attached resources (EC2, FSx, ELB, ENI, API Gateway). Results are logged and exported to a CSV file. A profile filter parameter allows filtering profiles by a regex pattern.
...
function Build-SecurityGroupAttachmentCache {
    ...
    try {
        ...
        do {
            ...
            foreach ($instance in $reservations.Instances) {
                foreach ($sg in $instance.SecurityGroups) {
                    Initialize-SgCache -sgId $sg.GroupId
                    $attachmentCache[$sg.GroupId].EC2Count += 1
                }
            }
        ...
        if ($_.Exception.Message -match "is not authorized") {
            Write-Log "Check if the profile has 'ec2:DescribeInstances' permission" "WARN"
        }
    }
    ...
    try {
        ...
        do {
            ...
            foreach ($eni in $enis) {
                foreach ($sg in $eni.Groups) {
                    Initialize-SgCache -sgId $sg.GroupId
                    $attachmentCache[$sg.GroupId].ENICount += 1
                }
            }
        ...
        if ($_.Exception.Message -match "is not authorized") {
            Write-Log "Check if the profile has 'ec2:DescribeNetworkInterfaces' permission" "WARN"
        }
    }
    ...
    try {
        ...
        do {
            ...
            foreach ($lb in $loadBalancers) {
                foreach ($sg in $lb.SecurityGroups) {
                    Initialize-SgCache -sgId $sg
                    $attachmentCache[$sg].ELBCount += 1
                }
            }
        ...
        if ($_.Exception.Message -match "is not authorized") {
            Write-Log "Check if the profile has 'elasticloadbalancing:DescribeLoadBalancers' permission" "WARN"
        }
    }
    ...
    try {
        ...
        do {
            ...
            foreach ($fs in $fileSystems) {
                foreach ($sg in $fs.NetworkInterfaceIds | ForEach-Object {
                    ...
                    Initialize-SgCache -sgId $sg.GroupId
                    $attachmentCache[$sg.GroupId].FSxCount += 1
                }
            }
        ...
        if ($_.Exception.Message -match "is not authorized") {
            Write-Log "Check if the profile has 'fsx:DescribeFileSystems' or 'ec2:DescribeNetworkInterfaces' permission" "WARN"
        }
    }
    ...
    try {
        ...
        do {
            ...
            foreach ($endpoint in $vpcEndpoints | Where-Object { $_.ServiceName -like "com.amazonaws.*.execute-api" }) {
                foreach ($sg in $endpoint.Groups) {
                    Initialize-SgCache -sgId $sg.GroupId
                    $attachmentCache[$sg.GroupId].APIGWCount += 1
                }
            }
    ...
    # Check Transit Gateway Attachments (contributes to ENI count)
    try {
        $nextToken = $null
        do {
            $params = @{
            ...
            foreach ($attachment in $tgwAttachments) {
                if ($attachment.ResourceType -eq 'vpc' -and $attachment.ResourceId) {
                    # Get subnets in the attached VPC
                    $subnets = Get-EC2Subnet -Filter @{Name="vpc-id";Values=$attachment.ResourceId} -ProfileName $ProfileName -Region $Region -ErrorAction Stop
                    foreach ($subnet in $subnets) {
                        $eniParams = @{
                            ProfileName = $ProfileName
                            Region = $Region
                            Filter = @{Name="subnet-id";Values=$subnet.SubnetId}
                            ErrorAction = 'Stop'
                        }
                        $enis = Get-EC2NetworkInterface @eniParams
                        foreach ($eni in $enis) {
        ...
        if ($_.Exception.Message -match "is not authorized") {
            Write-Log "Check if the profile has 'ec2:DescribeTransitGatewayAttachments' or 'ec2:DescribeNetworkInterfaces' permission" "WARN"
        }
    }
    ...
    try {
        ...
        do {
            ...
            foreach ($sg in $securityGroups) {
                ...
                $isAttached = $counts -and ($counts.EC2Count -gt 0 -or $counts.FSxCount -gt 0 -or $counts.ELBCount -gt 0 -or $counts.ENICount -gt 0 -or $counts.APIGWCount -gt 0)
                $securityGroupsOutput += [PSCustomObject]@{
                    ...
                    EC2Count         = if ($counts) { $counts.EC2Count } else { 0 }
                    FSxCount         = if ($counts) { $counts.FSxCount } else { 0 }
                    ELBCount         = if ($counts) { $counts.ELBCount } else { 0 }
                    ENICount         = if ($counts) { $counts.ENICount } else { 0 }
                    APIGWCount       = if ($counts) { $counts.APIGWCount } else { 0 }
...
try {
    ...
    Write-Host "==============================" -ForegroundColor Cyan
    Write-Host "Script version: 1.7 (Added EC2, FSx, ELB, ENI, APIGW usage counts)" -ForegroundColor Green
